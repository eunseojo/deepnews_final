...
def get_cwob_score_labels(dict1, old_dir):
    ...
    tech = np.load(os.path.join(old_dir,"data_techcrunch.npy"))
    guard = np.load(os.path.join(old_dir,"guardian_texts.npy"))
    np.random.shuffle(tech)
    np.random.shuffle(guard)
    all_x = [] 
    ...
    for _ in range(5000):
        for row in tech:
            ...
            for token in row:
                if token in dict1:
                    num_words += 1
                    token_sum = list(map(add, dict1[token], token_sum))
            if num_words != 0:
...
def cbow_glove(root_path, pretrained_vocab_path, pickle_path):
    ...
    for set_type in ["new_test"]:
        all_x, lengths, yhat_list = get_cwob_score_labels(dict1, root_path)
