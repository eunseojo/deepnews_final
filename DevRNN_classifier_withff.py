...
class RNN_classifier():
    def __init__(self,input_directory, pretrained, config):
        self.config = config
        xavier1 = tf.contrib.layers.xavier_initializer()
#        self.pretrained_embeddings = tf.get_variable(initializer = xavier1, shape = [self.config.vocab_size,self.config.embed_size],name = "pretrained_embeddings")   ###pretrained embeddings (vocab x embedding_size)
...
#        lstm_cell = tf.contrib.rnn.MultiRNNCell([lstm_cell1]*2)
        ...
        W_0 = tf.get_variable("W_0", initializer = xavier, shape = [self.config.hidden_size, self.config.hidden_size],dtype=tf.float64)
        b_0 = tf.get_variable("b_0", initializer=tf.zeros(shape = [1,self.config.hidden_size],dtype=tf.float64))
        W_1 = tf.get_variable("W_1", initializer = xavier, shape = [self.config.hidden_size, self.config.hidden_size],dtype=tf.float64)
        ...
        #last_output = tf.reduce_mean(outputs,axis=0)
        ...
        z0 = tf.matmul(outputs, W_0) + b_0
        a0 = tf.nn.relu(z0)
        z1 = tf.matmul(a0, W_1) + b_1
        a1 = tf.nn.relu(z1)
        z = tf.matmul(a1, W_y) + b_2
    ...
    def calculate_dev(self, sess,fileprint):
        ...
        for i in range(self.config.dev_max_num_batches):
            ...
            pred, loss = sess.run([self.pred, self.loss], feed_dict={self.input_placeholder: X, self.labels_placeholder: Y})
        ...
        with tf.Session() as session:
            ...
            for i in range(self.config.n_epochs):
                ...
                for j in range(self.num_batches):   ### one full epoch iteration through the doc generator
                    ...
                    epoch_accu_list.append(accuracy_per_batch)
                ...
                print("epoch ",i," loss is ",sum(epoch_accu_list)/len(epoch_accu_list))
